...
def load_raw(file_path):
    ...
    raw_image = np.fromfile(file_path, dtype=np.uint32)
...
def rggb2raw(rggb_image):
    
    height, width = rggb_image.shape[0]*2, rggb_image.shape[1]*2
    raw_image = np.zeros([height,width], dtype=np.uint32)
    
    raw_image[0:height:2, 0:width:2] = rggb_image[:,:,0]
    raw_image[0:height:2, 1:width:2] = rggb_image[:,:,1]
    raw_image[1:height:2, 0:width:2] = rggb_image[:,:,2]
    raw_image[1:height:2, 1:width:2] = rggb_image[:,:,3]
...
def convert_image(
    model, img_rgb, num_heads=10, sample_size=(64, 64), context_size=(128, 128), gammas=[1], batch_size=64, head_output=False
):
    model.eval()
    ...
    if head_output:
        out_rggb_heads = [np.zeros((img_rggb_h, img_rggb_w, 4), dtype=np.float32) for _ in range(num_heads)]
    ...
    try:
        while r < img_rgb_h:
            ...
            while c < img_rgb_w:
                ...
                if len(indexes) == batch_size or (
                                                c + sample_size[1] >= img_rgb_w and 
                                                r + sample_size[0] >= img_rgb_h):
                    # compute
                    ...
                    y, outputs, scaling = model(samples, global_imgs) # y:[128, 4, 32, 32] outputs:[128, 40, 32, 32] scaling: [128,10]

                    print(f"模型输出 - y: {y.shape}, outputs: {outputs.shape}, scaling: {scaling.shape}")
                    #print(scaling[:,0])
                    #exit()
                    y = y.detach().cpu().numpy()
                    y = y.transpose(0, 2, 3, 1)
                    patch_shape = y.shape[0]

                    # #### gamma head output
                    if head_output:
                        #outputs = outputs.detach().cpu().numpy()
                        gammahead_outputs = outputs.view(patch_shape, 10, 4, 32, 32)
                        ghead_o = gammahead_outputs.permute(1,0,3,4,2) # [10,patch_shape,32,32,4]
                        ghead_o = ghead_o.detach().cpu().numpy()
                        for head_idx in range(num_heads):
                            for i, idx in enumerate(indexes):
                                h, w = idx
                                out_rggb_heads[head_idx][h : h + sample_size_half[0], w : w + sample_size_half[1]] = ghead_o[head_idx, i]
        ...
        for head_no in range(len(gammas)):
            scaling_coefficients[f"gamma {gammas[head_no]}"]=scaling[0,head_no]

        print('converted rggb shape in method: ', out_rggb.shape)
        print('scaling coe: ', scaling_coefficients)

        if head_output:
            out_rggb_heads = [np.clip(img, 0, 1) for img in out_rggb_heads]
            return out_rggb, out_rggb_heads
        else:
            print('no head_output')
            return out_rggb, None
